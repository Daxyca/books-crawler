...
async def test_ensure_indexes(storage):
    ...
    assert storage.books_collection.create_index.call_count >= 1


@pytest.mark.asyncio
async def test_insert_book_new(storage):
    storage.books_collection.update_one.return_value.upserted_id = "abc123"
    storage.books_collection.update_one.return_value.modified_count = 0

    result = await storage.insert_book(SAMPLE_BOOK)
    assert result == "abc123"
    storage.books_collection.update_one.assert_called_once()


@pytest.mark.asyncio
async def test_insert_book_updated(storage):
    storage.books_collection.update_one.return_value.upserted_id = None
    storage.books_collection.update_one.return_value.modified_count = 1
...
async def test_get_book_by_url_found(storage):
    storage.books_collection.find_one.return_value = SAMPLE_BOOK.model_dump()
...
async def test_get_book_by_url_not_found(storage):
    storage.books_collection.find_one.return_value = None
...
async def test_count_books(storage):
    storage.books_collection.count_documents.return_value = 7
...
async def test_clear_all_books(storage):
    storage.books_collection.delete_many.return_value.deleted_count = 3
    await storage.clear_all_books()
    storage.books_collection.delete_many.assert_called_once()
